...
- Créer l'application avec le bundle ID `com.beloucif.blackout`
...
keytool -genkey -v -keystore blackout-release.jks ^
...
$keystore = Get-Content -Path "chemin/vers/blackout-release.jks" -Encoding Byte
$base64 = [Convert]::ToBase64String($keystore)
$base64 | Set-Content -Path "keystore-base64.txt"

# Ou sur Git Bash :
cat chemin/vers/blackout-release.jks | base64 > keystore-base64.txt
...
gh secret set ANDROID_KEY_ALIAS --body "blackout"
