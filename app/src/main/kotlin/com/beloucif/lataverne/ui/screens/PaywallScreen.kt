...
import com.beloucif.lataverne.core.PremiumPlan
import com.beloucif.lataverne.ui.theme.LaTaverneColors
import kotlinx.coroutines.launch

/**
 * "La Taverne Premium" paywall: 3 plans (lifetime highlighted as the default, our
    ...
    Column(
        ...
        Row(
            ...
            Box(
                modifier = Modifier
                    .size(48.dp)
                    .background(LaTaverneColors.Premium.copy(alpha = 0.12f), RoundedCornerShape(24.dp)),
                contentAlignment = Alignment.Center,
            ) {
                Icon(Icons.Filled.Lock, contentDescription = null, tint = LaTaverneColors.Premium)
            }
            IconButton(onClick = ::close) {
                Icon(Icons.Filled.Close, contentDescription = stringResource(R.string.paywall_close), tint = LaTaverneColors.InkMuted)
            }
        }
        ...
        LazyColumn(
            ...
            items(premiumCatalog, key = { it.id }) { entry ->
                Row(
                    ...
                    Text(entry.title, style = MaterialTheme.typography.bodyMedium, color = LaTaverneColors.Ink)
                    Text(
        ...
        Button(
            ...
            if (purchasing) {
                CircularProgressIndicator(modifier = Modifier.size(20.dp), color = LaTaverneColors.Bg)
            } else {
                Text(
...
private fun PlanRow(plan: PremiumPlan, selected: Boolean, onSelect: () -> Unit) {
    ...
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .background(
                if (selected) LaTaverneColors.Premium.copy(alpha = 0.10f) else LaTaverneColors.Surface,
        ...
        Column {
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text(plan.label, style = MaterialTheme.typography.titleMedium, color = LaTaverneColors.Ink, fontWeight = FontWeight.Bold)
                if (isBestValue) {
                    Text(
        ...
        Text(plan.priceLabel, style = MaterialTheme.typography.titleLarge, color = LaTaverneColors.Ink)
    }
}
