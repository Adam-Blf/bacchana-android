...
/**
 * "La Tournée Premium" paywall: 3 plans (lifetime highlighted as the default, our
    ...
    Column(
        ...
        Row(
            ...
            Box(
                modifier = Modifier
                    .size(48.dp)
                    .background(LaTourneeColors.Premium.copy(alpha = 0.12f), RoundedCornerShape(24.dp)),
                contentAlignment = Alignment.Center,
            ) {
                Icon(Icons.Filled.Lock, contentDescription = null, tint = LaTourneeColors.Premium)
            }
            IconButton(onClick = ::close) {
                Icon(Icons.Filled.Close, contentDescription = stringResource(R.string.paywall_close), tint = LaTourneeColors.InkMuted)
            }
        }
        ...
        LazyColumn(
            ...
            items(premiumCatalog, key = { it.id }) { entry ->
                Row(
                    ...
                    Text(entry.title, style = MaterialTheme.typography.bodyMedium, color = LaTourneeColors.Ink)
                    Text(
        ...
        Button(
            ...
            if (purchasing) {
                CircularProgressIndicator(modifier = Modifier.size(20.dp), color = LaTourneeColors.Bg)
            } else {
                Text(
...
private fun PlanRow(plan: PremiumPlan, selected: Boolean, onSelect: () -> Unit) {
    ...
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .background(
                if (selected) LaTourneeColors.Premium.copy(alpha = 0.10f) else LaTourneeColors.Surface,
        ...
        Column {
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text(plan.label, style = MaterialTheme.typography.titleMedium, color = LaTourneeColors.Ink, fontWeight = FontWeight.Bold)
                if (isBestValue) {
                    Text(
        ...
        Text(plan.priceLabel, style = MaterialTheme.typography.titleLarge, color = LaTourneeColors.Ink)
    }
}
