...
import com.beloucif.meskova.core.PremiumPlan
import com.beloucif.meskova.ui.theme.MeskovaColors
import kotlinx.coroutines.launch

/**
 * "Meskova Premium" paywall: 3 plans (lifetime highlighted as the default, our
    ...
    Column(
        ...
        Row(
            ...
            Box(
                modifier = Modifier
                    .size(48.dp)
                    .background(MeskovaColors.Premium.copy(alpha = 0.12f), RoundedCornerShape(24.dp)),
                contentAlignment = Alignment.Center,
            ) {
                Icon(Icons.Filled.Lock, contentDescription = null, tint = MeskovaColors.Premium)
            }
            IconButton(onClick = ::close) {
                Icon(Icons.Filled.Close, contentDescription = stringResource(R.string.paywall_close), tint = MeskovaColors.InkMuted)
            }
        }
        ...
        LazyColumn(
            ...
            items(premiumCatalog, key = { it.id }) { entry ->
                Row(
                    ...
                    Text(entry.title, style = MaterialTheme.typography.bodyMedium, color = MeskovaColors.Ink)
                    Text(
        ...
        Button(
            ...
            if (purchasing) {
                CircularProgressIndicator(modifier = Modifier.size(20.dp), color = MeskovaColors.Bg)
            } else {
                Text(
...
private fun PlanRow(plan: PremiumPlan, selected: Boolean, onSelect: () -> Unit) {
    ...
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .background(
                if (selected) MeskovaColors.Premium.copy(alpha = 0.10f) else MeskovaColors.Surface,
        ...
        Column {
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text(plan.label, style = MaterialTheme.typography.titleMedium, color = MeskovaColors.Ink, fontWeight = FontWeight.Bold)
                if (isBestValue) {
                    Text(
        ...
        Text(plan.priceLabel, style = MaterialTheme.typography.titleLarge, color = MeskovaColors.Ink)
    }
}
