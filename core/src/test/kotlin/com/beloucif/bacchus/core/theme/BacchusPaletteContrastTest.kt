...
/**
 * Mechanical WCAG 2.1 contrast guard for the Bacchus palette. Every pair below reads its colors
 * straight off [BacchusPalette.Light] / [BacchusPalette.Dark] - the exact same objects
 ...
 *
 * Real bug this guards against (reported twice by Adam while playing the app): [BacchusPalette.ink]
 ...
 */
class BacchusPaletteContrastTest {

    private data class Pair(
        val label: String,
        val fg: (BacchusPalette) -> PaletteColor,
        val bg: (BacchusPalette) -> PaletteColor,
    ...
    @Test
    fun `every ink-on-fill pair used by the app clears WCAG AA in light theme`() = assertAllPass(BacchusPalette.Light, "clair")

    @Test
    fun `every ink-on-fill pair used by the app clears WCAG AA in dark theme`() = assertAllPass(BacchusPalette.Dark, "sombre")

    private fun assertAllPass(palette: BacchusPalette, themeName: String) {
     ...
     * property the rest of this file relies on to justify [BacchusPalette.tileInk]/[BacchusPalette.onStatus]
    ...
    fun `regression fixture - ink on popYellow in dark theme stays below AA`() {
        val ratio = WcagContrast.ratio(BacchusPalette.Dark.ink, BacchusPalette.Dark.popYellow)
        assertTrue(
     ...
     * divider/border blended over [BacchusPalette.bg] at [BacchusPalette.borderAlpha]. Docs
     ...
     * pre-existing) is intentionally left out here: [BacchusPalette.borderStrong] (opaque,
    ...
    fun `dark theme border alpha clears the 3-to-1 non-text UI floor`() {
        val ratio = WcagContrast.blendedRatio(BacchusPalette.Dark.border, BacchusPalette.Dark.borderAlpha, BacchusPalette.Dark.bg)
        assertTrue(
