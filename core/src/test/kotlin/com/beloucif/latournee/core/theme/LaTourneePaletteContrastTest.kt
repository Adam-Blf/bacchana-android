package com.beloucif.latournee.core.theme

import org.junit.Assert.assertTrue
import org.junit.Test

/**
 * Mechanical WCAG 2.1 contrast guard for the La Tournee palette. Every pair below reads its colors
 * straight off [LaTourneePalette.Light] / [LaTourneePalette.Dark] - the exact same objects
 ...
 *
 * Real bug this guards against (reported twice by Adam while playing the app): [LaTourneePalette.ink]
 ...
 */
class LaTourneePaletteContrastTest {

    private data class Pair(
        val label: String,
        val fg: (LaTourneePalette) -> PaletteColor,
        val bg: (LaTourneePalette) -> PaletteColor,
    ...
    @Test
    fun `every ink-on-fill pair used by the app clears WCAG AA in light theme`() = assertAllPass(LaTourneePalette.Light, "clair")

    @Test
    fun `every ink-on-fill pair used by the app clears WCAG AA in dark theme`() = assertAllPass(LaTourneePalette.Dark, "sombre")

    private fun assertAllPass(palette: LaTourneePalette, themeName: String) {
     ...
     * property the rest of this file relies on to justify [LaTourneePalette.tileInk]/[LaTourneePalette.onStatus]
    ...
    fun `regression fixture - ink on popYellow in dark theme stays below AA`() {
        val ratio = WcagContrast.ratio(LaTourneePalette.Dark.ink, LaTourneePalette.Dark.popYellow)
        assertTrue(
     ...
     * divider/border blended over [LaTourneePalette.bg] at [LaTourneePalette.borderAlpha]. Docs
     ...
     * pre-existing) is intentionally left out here: [LaTourneePalette.borderStrong] (opaque,
    ...
    fun `dark theme border alpha clears the 3-to-1 non-text UI floor`() {
        val ratio = WcagContrast.blendedRatio(LaTourneePalette.Dark.border, LaTourneePalette.Dark.borderAlpha, LaTourneePalette.Dark.bg)
        assertTrue(
