...
/**
 * Mechanical WCAG 2.1 contrast guard for the Meskova palette. Every pair below reads its colors
 * straight off [MeskovaPalette.Light] / [MeskovaPalette.Dark] - the exact same objects
 ...
 *
 * Real bug this guards against (reported twice by Adam while playing the app): [MeskovaPalette.ink]
 ...
 */
class MeskovaPaletteContrastTest {

    private data class Pair(
        val label: String,
        val fg: (MeskovaPalette) -> PaletteColor,
        val bg: (MeskovaPalette) -> PaletteColor,
    ...
    @Test
    fun `every ink-on-fill pair used by the app clears WCAG AA in light theme`() = assertAllPass(MeskovaPalette.Light, "clair")

    @Test
    fun `every ink-on-fill pair used by the app clears WCAG AA in dark theme`() = assertAllPass(MeskovaPalette.Dark, "sombre")

    private fun assertAllPass(palette: MeskovaPalette, themeName: String) {
     ...
     * property the rest of this file relies on to justify [MeskovaPalette.tileInk]/[MeskovaPalette.onStatus]
    ...
    fun `regression fixture - ink on popYellow in dark theme stays below AA`() {
        val ratio = WcagContrast.ratio(MeskovaPalette.Dark.ink, MeskovaPalette.Dark.popYellow)
        assertTrue(
     ...
     * divider/border blended over [MeskovaPalette.bg] at [MeskovaPalette.borderAlpha]. Docs
     ...
     * pre-existing) is intentionally left out here: [MeskovaPalette.borderStrong] (opaque,
    ...
    fun `dark theme border alpha clears the 3-to-1 non-text UI floor`() {
        val ratio = WcagContrast.blendedRatio(MeskovaPalette.Dark.border, MeskovaPalette.Dark.borderAlpha, MeskovaPalette.Dark.bg)
        assertTrue(
